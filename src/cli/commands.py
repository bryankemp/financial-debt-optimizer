...
def analyze(
    ...
    try:
        ...
        optimizer = DebtOptimizer(debts, income_sources, recurring_expenses, future_income, future_expenses, settings)
        ...
        click.echo(f"  Minimum Payments: ${debt_summary['total_minimum_payments']:,.2f}")
        click.echo(f"  Available Cash Flow: ${debt_summary['available_cash_flow']:,.2f}")
        click.echo(f"  Current Bank Balance: ${debt_summary['current_bank_balance']:,.2f}")
        click.echo(f"  Available Extra Payment: ${debt_summary['available_extra_payment']:,.2f}")
        
        if debt_summary['available_cash_flow'] < 0:
            click.echo("❌ Warning: Negative cash flow - income is less than minimum payments!", err=True)
        ...
        click.echo(f"  Time to Freedom: {result.total_months_to_freedom} months ({result.total_months_to_freedom // 12} years, {result.total_months_to_freedom % 12} months)")
        click.echo(f"  Interest Saved: ${result.savings_vs_minimum['interest_saved']:,.2f}")
        ...
        if compare_strategies:
            ...
            for _, row in strategy_comparison.iterrows():
                click.echo(f"  {row['strategy'].replace('_', ' ').title()}: "
        ...
        click.echo("Please check your input file and ensure all required data is present.", err=True)
        sys.exit(1)
    except Exception as e:
...
def validate(input_file: str):
    ...
    try:
        ...
        if not income_sources:
            ...
            total_monthly_income = sum(income.get_monthly_amount() for income in income_sources)
        ...
        click.echo(f"  Monthly Income: ${sum(income.get_monthly_amount() for income in income_sources):,.2f}")
...
def info():
    ...
    click.echo("   • Extra payments are automatically calculated from available cash flow")
