...
class TestDebtOptimizer:
    ...
    def setup_method(self):
        ...
        self.future_income = [
            FutureTransaction("Bonus", 2000.0, date(2025, 3, 15), "once"),
        ]
        
        self.future_expenses = [
            FutureTransaction("Car Repair", 800.0, date(2025, 2, 15), "once"),
        ]
    ...
    def test_debt_summary_generation(self):
        ...
        required_keys = [
            'total_debt', 'monthly_income', 'total_minimum_payments',
            'available_cash_flow', 'current_bank_balance', 'available_extra_payment'
        ]
    ...
    def test_cash_flow_calculation(self):
        ...
        monthly_expenses = sum(exp.get_monthly_amount() for exp in self.recurring_expenses)
    ...
    def test_hybrid_strategy(self):
        ...
        result = self.optimizer.optimize_debt_strategy(
            goal=OptimizationGoal.MAXIMIZE_CASHFLOW,
            extra_payment=150.0
        )
    ...
    def test_extra_payment_impact(self):
        ...
        assert result_with_extra.total_months_to_freedom <= result_no_extra.total_months_to_freedom
    ...
    def test_payment_schedule_format(self):
        ...
        expected_columns = ['month', 'debt_name', 'balance_before', 'interest_charge', 
                          'principal_payment', 'balance_after', 'total_payment']
    ...
    def test_future_transactions_impact(self):
        ...
        optimizer_with_future = DebtOptimizer(
            self.debts, self.income, self.recurring_expenses, 
            self.future_income, self.future_expenses, self.settings
        )
    ...
    def test_edge_case_zero_balance_debt(self):
        ...
        optimizer = DebtOptimizer(debts_with_zero, self.income, [], [], [], self.settings)
    ...
    def test_large_extra_payment(self):
        ...
        result = self.optimizer.optimize_debt_strategy(
            goal=OptimizationGoal.MINIMIZE_TIME,
            extra_payment=large_extra_payment
        )
    ...
    def test_optimization_consistency(self):
        ...
        for i in range(1, len(results)):
            ...
            assert abs(results[i].total_interest_paid - results[0].total_interest_paid) < 0.01
            assert results[i].total_months_to_freedom == results[0].total_months_to_freedom
